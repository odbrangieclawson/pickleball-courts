...
function makeCount(value, denominator, known, basis) {
 ...
 */
export function getCounts(scope, allVenues) {
  ...
  const verified = inside.filter(isVerified)
  ...
  const countTrue = field => makeCount(
    verified.filter(v => isTrue(v[field])).length, D, known(field), 'verified venues in scope',
  )
  const sumOf = field => {
    const withValue = verified.filter(v => typeof v[field] === 'number')
    return makeCount(
      withValue.reduce((a, v) => a + v[field], 0), D, withValue.length,
      'verified venues in scope that state this figure',
    )
  }

  return {
    venues: makeCount(D, D, D, 'verified venues in scope'),
    ...
    */
    venues_unverified: makeCount(
      ...
      'ALL venues in scope, verified or not',
    ),
  }
}
