...
import {slugifyCounty, STATE_NAMES} from '../../scripts/lib/us-geo.mjs'

export const stateName = st => STATE_NAMES[String(st).toUpperCase()] ?? String(st).toUpperCase()

/*
  Lower-case, and anything that is not a letter or digit becomes one hyphen.
  "St. George" was the city that found the old version (spaces only): it
  produced "st.-george", a URL with a dot in it. Every city published before
  it was letters and spaces, so nothing else changes.
*/
