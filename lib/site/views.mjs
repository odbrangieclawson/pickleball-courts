  ...
  const jsonLd = JSON.stringify({
    ...
    "@graph": [
      ...
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          {"@type": "ListItem", position: 1, name: "Deep Pickleball", item: `${ORIGIN}/`},
        ],
      },
    ],
  })
        ...
        */
        {
          ...
          creator: {'@type': 'Organization', name: 'Deep Pickleball', url: `${ORIGIN}/`},
          spatialCoverage: {
            '@type': 'State',
            name: s.stateName,
...
export function navView() {
  ...
  const groups = [...byState.values()]
    ...
    /* Most-covered state first, then alphabetical. */
    .sort((a, b) => b.cities.length - a.cities.length || a.label.localeCompare(b.label))
