...
*/
export function venuePagePublishes(slug) {
  const notes = venueEditorialIndex().get(slug)
  ...
  return {
    ...
    */
    ...
    bestFor: (editorial?.bestFor ?? []).map(b => ({
      ...
      href: b.venue_slug && venuePagePublishes(b.venue_slug) ? venueHref(c.state, c.slug, b.venue_slug) : null,
    })),
    hasFaqs: (editorial?.faqs ?? []).length > 0,
    faqs: (editorial?.faqs ?? []).map(f => ({q: f.q, a: f.a})),

    venuePagesN: fmt(c.venues.filter(v => venuePagePublishes(v.slug)).length),
    ...
    venues: c.venues
      ...
      .map(v => ({
        href: venuePagePublishes(v.slug) ? venueHref(c.state, c.slug, v.slug) : null,
        name: v.name,
...
export function venueView(stateSlug, citySlug, venueSlug) {
  ...
  if (!r) return null
  if (!venuePagePublishes(venueSlug)) return null
  const {venue: v, city: c, alternatives} = r

  const graph = linkGraph()
  const prov = v.field_provenance ?? {}
  const notes = editorialForVenue(venueEditorialIndex(), v.slug)
  ...
  */
  ...
  return {
    ...
    hasAlternatives: alternatives.some(a => venuePagePublishes(a.slug)),
    alternatives: alternatives.filter(a => venuePagePublishes(a.slug)).map(a => ({
  ...
  */
  ...
  return {
    ...
    venues: matching
      ...
      .map(v => ({
        slug: v.slug,
        href: venuePagePublishes(v.slug) ? venuePath(c.state, c.slug, v.slug) : null,
        name: v.name,
...
export function allLeafParams() {
  ...
  for (const c of data.publishedCities()) {
    ...
    for (const v of full.venues) {
      if (!venuePagePublishes(v.slug)) continue
...
export function countyView(stateSlug, countySlugWithSuffix) {
  ...
  return {
    ...
    venues: entry.venues
      ...
      .sort((a, b) => (b.total_courts ?? 0) - (a.total_courts ?? 0) || String(a.name).localeCompare(String(b.name)))
      .map(v => ({
        href: venuePagePublishes(v.slug) ? venuePath(st, citySlugOf(v.city), v.slug) : null,
        name: v.name,
        city: v.city,
        courts: v.total_courts === null || v.total_courts === undefined
          ? 'Not verified yet'
