...
import {search, venuesTotal} from './search.mjs'
import * as data from './data.mjs'
import {loadVerifiedOverlay} from '../data/verified.mjs'
import {ORIGIN} from './origin.mjs'
import {claimMailto, correctionMailto} from './contact.mjs'
import {courtPhoto, cityCourtPhoto, courtCardPhoto, proseWash} from './photos.mjs'
import {staticMap} from './maps.mjs'

/* See venueView's `map`: the venue-level tile set costs ~23 MB of repo. */
const VENUE_MAPS = false
import {readFileSync, existsSync} from 'node:fs'
import {join} from 'node:path'
import {REPO_ROOT as REPO_ROOT_FOR_OUTLINES} from '../../scripts/lib/load-csv.mjs'
...
/*
  ...
  We have none of our own. Nobody has photographed the venues on this site,
  so each one shows an openly licensed photograph of somebody ELSE's
  pickleball courts, chosen from its slug so the same venue always shows the
  same picture and a city grid does not repeat one image down the page.

  That is fine as a layout decision and dangerous as a silent one. A picture
  of a court on a venue page, unlabelled, tells a reader it is a photograph
  OF that court, which is a claim made by an image rather than by a
  sentence, and one no source supports. On a site that renders a missing
  court count as "not stated" it would be incoherent to let a
  photograph imply something the text would refuse to.

  So every one carries isPlaceholder, the templates mark it, the alt text
  says what it actually is, and the credit names the photographer. When real
  photographs arrive, lib/site/photos.mjs is where they attach and the
  marker disappears for the venues that have one.
*/

/*
  WHERE IT IS, AND HOW TO GET THERE.

  A directory that tells you a court exists and not how to reach it has
  done half the job. This builds a directions link for a venue.

  It is a plain anchor to a maps URL, not an embedded map. Rule 1 says every
  page works with JavaScript off, and an anchor does: on a phone it hands
  off to whichever map app is installed, on a desktop it opens the route.
  An embedded slippy map would need a script, a tile provider and a third
  party watching every visitor, to show something the reader is about to
  leave the page for anyway.

  Coordinates are preferred over the address, because they are what the
  geocoder resolved and checked against the incorporated place. 281 of the
  282 published venues have them. The one that does not falls back to its
  street address with the city and state appended, which is the same string
  a person would type. A venue with neither gets no button rather than a
  guess, which is the same rule the rest of the site follows.
*/
/* The host that served a source, for link text. Same rule as the city page. */
const hostOf = u => {
  if (!u) return null
  try { return new URL(u).hostname.replace(/^www\./, '') } catch { return u }
}

function directionsFor(v, c) {
  const hasGeo = typeof v.latitude === 'number' && typeof v.longitude === 'number'
  const destination = hasGeo
    ? `${v.latitude},${v.longitude}`
    : [v.street_address, c.city, c.state].filter(Boolean).join(', ')
  if (!destination) return null
  return {
    href: `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(destination)}`,
    /* An address can land on the block; a coordinate lands on the court. */
    precise: hasGeo,
  }
}

/*
  One of the licensed court photographs, picked from the venue's slug so it
  is stable across builds and identical on the card and the page. It is
  still a placeholder: none of these is a photograph of this venue, and the
  marker and the caption both say so.
*/
const photoFor = venue => {
  const p = courtPhoto(venue.slug, venue.name)
  return {
    ...p,
...
const cardDetail = v => [
  ...
  v.light === true ? 'lit for evening play' : v.light === false ? 'no lights' : 'lighting not stated',
  v.nets_provided === true ? 'nets provided' : v.nets_provided === false ? 'bring a net' : 'nets not stated',
...
export const feeWord = v => (v === null || v === undefined ? 'Not stated' : FEE_WORDS[v] ?? String(v))
...
export function homeView() {
  ...
  */
  ...
  return {
    ...
    /* For the About page. Read from siteTotals(), not from states.length in
       the page: the no-bypass scan refuses a page that measures a collection. */
    stateCount: fmt(t.stateCount),
    ...
    */
    ...
    cities: cities.map(c => ({
      ...
      /* A court, not the city. It is somebody else's court, so it carries
         the same "No photo yet" marker every venue card carries. A skyline
         here was a true picture and the wrong one: this is a directory of
         places to play. */
    ...
    statesHeading: 'Where we have verified, by state',
    /*
      THIS RANKS OUR COVERAGE, NOT THE STATES.

      The obvious version of this section is the one every competitor
      ships: "the most active states for pickleball", ranked by court
      count, over a photograph. We cannot make that claim and do not.
      Washington leads this list because five of its cities have been
      verified, not because it out-plays Florida. The heading and this
      sentence both say so, because a ranking with an honest label is
      useful and the same ranking with a dishonest one is a lie about the
      whole country.

      The example in the sentence is COMPUTED, not written. The first draft
      named Florida as "bottom but one" with "one city", and Florida is now
      #2 with three. check-claims.mjs cannot see prose in this file, so the
      sentence names whichever state is last today and reads its city count
      from the same cards the section renders.
    */
    statesNote: statesNoteFor(stateCards()),
    states: stateCards(),
  }
}

/*
  The home page's state cards.

  ============================================================
  WHY EVERY CARD IS NOT A LINK TO A STATE PAGE
  ============================================================

  Nine states have published venues and only two of them — NC and WA —
  have a state PAGE, because STATE_MIN_CITIES is 3 and the other seven
  publish one or two cities each. A grid of nine cards each linking to
  /pickleball/us/{state}/ would therefore ship seven hard 404s, which is
  precisely the failure lib/site/links.mjs was written after: a crawl once
  found 30 dead links from a single hard-coded nav href.

  So a card links to the state page when that page exists, and otherwise
  lists its cities, which always have pages. Every href on this section is
  minted from a page that was actually built. Nothing here can 404, and
  nothing here needs the state-page threshold relaxed to work — O13 stays
  an open decision rather than being quietly settled by a home-page
  feature that wanted a link.

  ============================================================
  WHY THERE ARE NO PHOTOGRAPHS
  ============================================================

  The competitor version of this section puts a photograph of a pickleball
  court behind each state name. We have no photographs of these places —
  venue pages say "No photo yet" for exactly this reason — and a stock
  court image captioned "Arizona" is a claim about Arizona that no source
  in this repository supports. A state outline would be honest, and there
  is no boundary data here to draw one accurately from; an approximated
  Texas is a factual error rendered at 200 pixels. So the mark on each
  card is the state's own postal abbreviation, which is a fact.
*/
let outlineIndex = null
function stateOutline(code) {
  if (!outlineIndex) {
    const f = join(REPO_ROOT_FOR_OUTLINES, 'data/reference/state-outlines.json')
    outlineIndex = existsSync(f) ? (JSON.parse(readFileSync(f, 'utf8')).states ?? {}) : {}
  }
  const o = outlineIndex[String(code).toUpperCase()]
  return o ? {viewBox: o.viewBox, d: o.d} : null
}

/*
  The sentence under the "by state" heading, built from the cards it sits
  above so it cannot disagree with them. It names the state at the bottom of
  the ranking and how many of its cities we have verified, which is the
  honest reading of a low rank: not "little pickleball there" but "we have
  barely started there".
*/
function statesNoteFor(cards) {
  const lead = 'Ranked by the courts we have verified, which measures our coverage rather than the states. '
  const last = cards[cards.length - 1]
  if (!last) return lead.trim()
  const n = last.cities.length
  const so = n === 1 ? 'one city' : `${fmt(n)} cities`
  return lead
    + `${last.stateName} is last here because we have verified ${so} in it so far, `
    + 'which says nothing about how much pickleball is played there.'
}

function stateCards() {
  const rows = []
  for (const s of data.publishedStates()) {
    const st = data.state(s.slug)
    if (!st) continue
    const venues = num(st.counts.venues)
    const courts = num(st.counts.courts)
    rows.push({
      key: s.state,
      mark: s.state,
      outline: stateOutline(s.state),
      stateName: s.stateName,
      courtsForSort: courts,
      venues: fmt(venues),
      courts: fmt(courts),
      meta: `${fmt(venues)} verified ${pluralise(venues, 'venue')} · ${fmt(courts)} ${pluralise(courts, 'court')}`,
      /* Null where the state publishes no page. The template branches on it. */
      /*
        EVERY STATE NAME LEADS SOMEWHERE.

        A state page needs three published cities and a written note, so
        only five of these nineteen have one. The other fourteen used to be
        dead text: the reader could see Texas had two cities listed under
        it and could not click the word Texas, with nothing on the card
        saying why.

        Search already resolves a state name to everything we hold in it —
        "Texas" returns Austin, San Antonio and both counties — so that is
        the honest destination while the state page does not exist. No new
        URL is minted and the three-city threshold is untouched; the card
        simply stops pretending there is nothing there.
      */
      href: statePagePublishes(s.state) ? stateHref(s.state) : `/search/?q=${encodeURIComponent(s.stateName)}`,
      /* Which of the two it is, so the card can say so. */
      hrefKind: statePagePublishes(s.state) ? 'state' : 'search',
      cityLabel: `${fmt(st.cities.length)} ${pluralise(st.cities.length, 'city', 'cities')}`,
  ...
  return rows
    .sort((a, b) => b.courtsForSort - a.courtsForSort || a.stateName.localeCompare(b.stateName))
    .map(({courtsForSort, ...card}, i) => ({...card, rank: `#${i + 1}`}))
}

/* ---------------------------------------------------------------- */

/*
  ONE PICTURE PER PARAGRAPH, AND WHY NOT PER SECTION.

  The wash began as one photograph per note, pinned to the top of the
  section. On a state page that leaves a single picture beside a heading and
  then eight hundred words of empty gutter under it, which is the same
  complaint it was built to answer, only further down the page.

  So a paragraph gets its own, and the picture column keeps pace with the
  text column all the way down.

  A PARAGRAPH HAS TO EARN ONE. The wash is stretched to the height of the
  paragraph it sits beside, so a two-line paragraph would get a two-line
  letterbox smear rather than a photograph. Below this many characters it
  gets nothing, which is why the opening line of a note — "California has
  fifty-eight counties. This page covers three cities in two of them." at
  82 characters — is left alone and the four substantial paragraphs under
  it are not.
*/
const WASH_MIN_CHARS = 220

/*
  A note's paragraphs, each with the photograph that belongs beside it.
  The blank line is the author's paragraph break; every note in the
  editorial store is written with them.
*/
const washedParagraphs = (scope, text) =>
  String(text ?? '').split(/\n\n+/).map((para, i) => ({
    text: para,
    wash: para.length >= WASH_MIN_CHARS ? proseWash(`${scope}/${i}`) : null,
  }))

/* Reader-facing headings for the four state slots. */
const STATE_HEADINGS = {
  coverage: 'What we cover in this state',
...
/*
  venue_type in the reading format. The enum names WHO OPERATES a venue and
  nothing else — not cost, not who may enter (see the note in
  data/vocabularies/venue-type.json). These labels must not drift into
  implying either, which is why "public_park" reads "Park" and not "Free"
  or "Public access".
*/
const VENUE_TYPE_LABEL = Object.freeze({
  public_park: 'Park',
  community_center: 'Rec centre',
  dedicated_pickleball_facility: 'Pickleball club',
  school: 'School',
...
/*
  A venue card for the state page. Every badge is a stated fact or it is
  absent: a venue whose court count nobody publishes gets no count badge
  rather than a zero, and one whose lighting is unstated gets no Lit badge
  rather than an unlit one. Rule 6 in badge form — the gap is silence here
  rather than the words "Not stated", because a row of grey "unknown" chips
  is noise where a missing chip is simply nothing to say.
*/
function stateVenueCard(v, c, st) {
  const inOut = (v.indoor_courts ?? 0) > 0 && (v.outdoor_courts ?? 0) > 0 ? 'In & Out'
    : (v.indoor_courts ?? 0) > 0 ? 'Indoor'
      : (v.outdoor_courts ?? 0) > 0 ? 'Outdoor'
        : null
  return {
    key: `${st}/${c.slug}/${v.slug}`,
    href: venueHref(st, c.slug, v.slug),
    name: v.name,
    where: `${c.city}, ${st}`,
    courts: typeof v.total_courts === 'number' ? `${fmt(v.total_courts)} ${pluralise(v.total_courts, 'court')}` : null,
    inOut,
    type: VENUE_TYPE_LABEL[v.venue_type] ?? null,
    lit: v.light === true,
    photo: photoFor(v),
  }
}
...
export function stateView(slug) {
  ...
  return {
    stateName: s.stateName,
    state: s.state,
    /* No landmark hero. The card image is a court; see photos.mjs. */
    photo: null,
    cardPhoto: courtCardPhoto(`ST/${s.state}`, s.stateName),

    /*
      Every published venue in the state, biggest first, as cards. A state
      page used to be a list of city cards — a directory of directories.
      These are the courts themselves, which is what somebody arriving on
      "pickleball courts in Kansas" came for.
    */
    venueCards: s.cities
      .flatMap(c => (c.venues ?? [])
        .filter(v => venuePagePublishes(s.state, c.slug, v.slug))
        .map(v => stateVenueCard(v, c, s.state)))
      .sort((a, b) => (parseInt(b.courts ?? '0', 10) || 0) - (parseInt(a.courts ?? '0', 10) || 0)
        || a.name.localeCompare(b.name)),

    /*
      Booleans, not lengths. Rule 2: a page must never measure a collection
      — the no-bypass scan fails the build on `.length` in app/, because
      that is a page computing its own number instead of reading one the
      data layer produced.
    */
    hasVenueCards: s.cities.some(c => (c.venues ?? []).some(v => venuePagePublishes(s.state, c.slug, v.slug))),
    hasCityChips: s.cities.length > 0,

    /*
      FILTER CHIPS, AND ONLY THE ONES THIS STATE CAN ANSWER.

      A chip appears only where at least one city in the state publishes
      that filter page, so Kansas offers Indoor and Washington offers Lit
      because those cities exist, and neither offers Free because no
      operator in either states a price. A chip for a filter with nothing
      behind it would be a promise of courts we cannot show.

      They point at /search/?q=<state>&filter=<slug>. Rule 4 caps indexable
      filter pages at five PER CITY, so a state-level facet cannot be a
      page — and Rule 4 says every other facet is a noindex query
      parameter, which is exactly what this is. No new URL, no rule bent.

      The count is of CITIES, not courts, and the label says so. Summing
      the venues behind these pages would be a number about our coverage
      wearing the clothes of a number about the state.
    */
    filterChips: (() => {
      const byFilter = new Map()
      for (const c of s.cities) {
        for (const f of Object.keys(qualifyingFilters(c.venues ?? []))) {
          byFilter.set(f, (byFilter.get(f) ?? 0) + 1)
        }
      }
      return FILTERS
        .filter(f => byFilter.has(f))
        .map(f => ({
          filter: f,
          label: QUICK_LINK_HEADING[f] ?? FILTER_LABEL[f],
          cities: byFilter.get(f),
          href: `/search/?q=${encodeURIComponent(s.stateName)}&filter=${f}`,
        }))
    })(),
    hasFilterChips: s.cities.some(c => Object.keys(qualifyingFilters(c.venues ?? [])).length > 0),

    /* Shortcut chips under the search box: the published cities, largest
       first, so the reader can skip the scroll. */
    cityChips: s.cities
      .slice()
      .sort((a, b) => (b.venues?.length ?? 0) - (a.venues?.length ?? 0))
      .map(c => ({label: c.city, href: cityHref(s.state, c.slug)})),
    title: `Pickleball Courts in ${s.stateName}`,
    meta: `${fmt(num(s.counts.venues))} pickleball venues across ${fmt(s.cities.length)} ${pluralise(s.cities.length, 'city', 'cities')} in ${s.stateName}, each with its source and the date it was checked.`,
    ...
    cities: s.cities.map(c => ({
      ...
      meta: `${fmt(c.venueCount)} ${pluralise(c.venueCount, 'venue')} · ${fmt(c.courtCount)} ${pluralise(c.courtCount, 'court')}`,
      blurb: `Court counts, lights and addresses for every ${c.city} venue.`,
    })),
    ...
    editorial: doc
      ? STATE_SLOTS.filter(x => doc.slots?.[x.key]).map(x => ({
        ...
        paragraphs: washedParagraphs(`ST/${s.state}/${x.key}`, doc.slots[x.key]),
        sources: (doc.sources ?? [])
    ...
    jsonLd: JSON.stringify({
      ...
      '@graph': [
        ...
        */
        {
          '@type': 'Dataset',
          name: `Pickleball courts in ${s.stateName}`,
...
export function cityView(stateSlug, citySlug) {
  ...
  return {
    ...
    county: c.county,
    title: `Pickleball Courts in ${c.city}, ${c.state}: ${fmt(num(c.counts.venues))} ${pluralise(num(c.counts.venues), 'Venue')}`,
    meta: `${fmt(num(c.counts.venues))} pickleball ${pluralise(num(c.counts.venues), 'venue')} in ${c.city}, ${c.state}: court counts, lights and addresses, each with its source and the date it was checked.`,
    h1: `Pickleball Courts in ${c.city}, ${c.state}`,
    /*
      No landmark photograph on a city page any more. The picture a reader
      wants here is a court, and a skyline above a list of courts was
      decoration standing where the map should go. `cardPhoto` is not
      rendered on the page at all: it is the share and search-result image
      only, so the thing that shows up beside "Pickleball Courts in Wichita"
      in a result list or a pasted link is a court.
    */
    photo: null,
    cardPhoto: cityCourtPhoto(c.state, c.slug, c.city),
    ...
    editorial: editorial
      ? EDITORIAL_SLOTS
        ...
        .map(s => ({
          ...
          paragraphs: washedParagraphs(`CI/${c.state}/${citySlug}/${s.key}`, editorial.slots[s.key]),
    ...
    */
    ...
    venues: c.venues
      ...
      .map(v => ({
        ...
        indoorOutdoor: (v.outdoor_courts ?? 0) > 0 && (v.indoor_courts ?? 0) > 0 ? 'Both'
          : (v.outdoor_courts ?? 0) > 0 ? 'Outdoor'
            : (v.indoor_courts ?? 0) > 0 ? 'Indoor' : 'Not stated',
        lights: triWord(v.light),
        nets: triWord(v.nets_provided),
        fee: feeWord(v.fee_type),
        address: v.street_address ?? 'Not stated',
    ...
    /*
      THE MAP. Every venue we hold a geocode for, as a pin over tiles we
      serve ourselves. Venues without coordinates are absent rather than
      stacked at the origin, and the caption below says how many, because a
      map that quietly drops three venues is a map that lies about coverage.
    */
    map: staticMap(
      c.venues.map(v => ({
        latitude: v.latitude,
      ...
      {width: 720, height: 400, maxZoom: 14},
    ),
    mappedN: c.venues.filter(v => typeof v.latitude === 'number' && typeof v.longitude === 'number').length,
...
export function venueView(stateSlug, citySlug, venueSlug) {
  ...
  /*
    Some facts are a word or a number and sit on one line with a dotted
    leader; some are a sentence or a paragraph an operator wrote, and those
    cannot share a three-column grid with the short ones — Tampa's
    Macfarlane Park had its court-availability prose running across the
    Lights and Nets columns. The wide ones are marked here and rendered
    full-width, after the short ones.
  */
  const WIDE = new Set(['pricing_notes', 'hours_of_operation', 'court_availability', 'level_of_play', 'parking', 'amenities', 'website'])
  const row = (key, label, value) => ({
    ...
    /*
      The publisher, for the link text in "Where each fact came from".

      The template used to hardcode "Seattle Parks open data" on every row of
      every venue in every city, so a Mesa page credited Seattle while linking
      to mesaaz.gov. On a site whose whole promise is that each fact names the
      organisation that published it, a wrong name under a right link is the
      worst of both.

      field_provenance stores a URL and no publisher name, so the honest label
      is the host that served it. lib/page/city-page.mjs already labels its
      source column this way; this makes the venue page agree.
    */
    sourceLabel: hostOf(prov[key]?.source_url ?? null),
    checked: prov[key]?.date_checked ?? null,
  })

  const n = x => (x === null || x === undefined ? 'Not stated' : fmt(x))
  const money = x => (x === null || x === undefined ? 'Not stated' : `$${fmt(x)}`)
  const text = x => (x === null || x === undefined || x === '' ? 'Not stated' : String(x))
  const list = x => (Array.isArray(x) && x.length ? x.join(', ') : 'Not stated')
  ...
  */
  ...
  const known = facts.filter(f => f.value !== 'Not stated').length
  ...
  */
  const ladder =
    v.verified_by === 'municipal_source' ? {rank: 1, label: `From the operator's own records, ${v.date_checked}`}
      : v.claimed_by_owner ? {rank: 2, label: `Confirmed by the venue, ${v.claim_date ?? 'date unknown'}`}
        : v.source_url ? {rank: 3, label: 'Listed from public data, not yet checked against the operator'}
          : {rank: 4, label: 'No source on record'}
  ...
  */
  ...
  return {
    ...
    /*
      Under 155 characters with the longest venue name on the site, and no
      dash. The court count leads when it is verified (D8: verified counts
      only); when it is null the sentence simply does not mention one.
    */
    meta: v.total_courts === null || v.total_courts === undefined
      ? `Pickleball at ${v.name} in ${c.city}, ${c.state}. Courts, lights, nets and address, with a source and date for each.`
      : `${fmt(v.total_courts)} pickleball ${pluralise(v.total_courts, 'court')} at ${v.name} in ${c.city}, ${c.state}. Lights, nets and address, with a source and date for each.`,
    ...
    photo: photoFor(v),
    directions: directionsFor(v, c),
    streetAddress: v.street_address ?? null,

    /*
      One pin, as close as the tiles go — and OFF by default.

      Tiles live in the repository, and that is the whole price of owning
      them instead of renting them. City maps cost 322 tiles, about 7 MB,
      and they are the competitive gap: a directory that cannot show you
      where the courts are is a list. Venue maps cost a further 1,055 tiles
      at zoom 16, about 23 MB, because at that zoom almost no two venues
      share a tile — three times the weight for a picture that sits beside
      a "Get directions" button already doing the same job.

      Flip VENUE_MAPS to true and run `npm run map:tiles` to have them.
    */
    map: VENUE_MAPS
      ? staticMap(
        [{latitude: v.latitude, longitude: v.longitude, slug: v.slug, name: v.name}],
        {width: 300, height: 220, maxZoom: 16},
      )
      : null,
    ...
    /*
      ...
      A shortcut, not a second fact table. The count, the cost and the hours
      are the three things a player checks before leaving the house, and the
      three actions — route me there, ring them, open their page — are what
      they do next. Everything here also appears in the full fact panel
      below, which is where Rule 6 lives: every field renders whether or not
      it is known. So cost and hours still say "Not stated" out loud here,
      because a player who does not check is a player who turns up to a
      locked gym. The two links are the exception: an action with no target
      is not an honest gap, it is a dead button, so they are absent rather
      than disabled.

      No opening hours are invented and no fee is assumed. Where the operator
      has not published one, this panel says so in the same words the table
      does.
    */
    panel: {
      courts: v.total_courts ?? null,
      /* The split only reads as a split when at least one side is known. */
      split: (v.indoor_courts ?? null) === null && (v.outdoor_courts ?? null) === null
        ? null
        : [
          (v.indoor_courts ?? null) === null ? null : `${fmt(v.indoor_courts)} indoor`,
          (v.outdoor_courts ?? null) === null ? null : `${fmt(v.outdoor_courts)} outdoor`,
        ].filter(Boolean).join(' · '),
      cost: v.drop_in_fee_usd !== null && v.drop_in_fee_usd !== undefined
        ? `${feeWord(v.fee_type)}, $${fmt(v.drop_in_fee_usd)}`
        : feeWord(v.fee_type),
      hours: v.hours_of_operation ? String(v.hours_of_operation) : 'Not stated',
      phone: v.phone ?? null,
      /* tel: wants digits. A US number keeps its country code so the dial
         works from abroad and from a saved contact. */
      phoneHref: v.phone ? `tel:+1${String(v.phone).replace(/\D/g, '').replace(/^1/, '')}` : null,
      website: v.website ?? null,
      websiteLabel: hostOf(v.website ?? null),
    },
    ...
    /* Both go to the same inbox with a subject the queue can sort on. The
       venue URL rides in the body so a reply never has to ask which one. */
    claimHref: claimMailto({name: v.name, city: c.city, state: c.state, url: `${ORIGIN}${venuePath(c.state, c.slug, v.slug)}`}),
    correctionHref: correctionMailto({name: v.name, city: c.city, state: c.state, url: `${ORIGIN}${venuePath(c.state, c.slug, v.slug)}`}),
    checked: v.date_checked ?? null,
    source: v.source_url ?? null,
      ...
      '@graph': [
        ...
        },
        {
          ...
          ...(typeof v.latitude === 'number' && typeof v.longitude === 'number'
            ...
            : {}),
          /* hasMap is the schema counterpart of the Get directions button:
             the same destination, declared for a machine rather than a reader. */
          ...(directionsFor(v, c) ? {hasMap: directionsFor(v, c).href} : {}),
...
const FILTER_EXCLUSIONS = {
  ...
  outdoor: 'Nothing with only indoor courts appears here. Venues whose outdoor court count the operator does not state are also absent, because we do not assume a court is outdoor because we have not been told otherwise.',
  free: 'Drop-in sessions with a door fee are not free and are not listed. Neither is any venue with a membership price above zero, however low. A venue whose cost the operator does not state stays off this list rather than being assumed free.',
...
export function filterView(stateSlug, citySlug, filter) {
  ...
  */
  ...
  return {
    ...
    meta: `${fmt(num(count))} pickleball ${pluralise(num(count), 'venue')} in ${c.city}, ${c.state} ${FILTER_PREDICATE_TEXT[filter]}, each with its source and the date it was checked.`,
...
export function countyView(stateSlug, countySlugWithSuffix) {
  ...
  return {
    ...
    slug: `${bare}-county`,
    title: `Pickleball Courts in ${entry.county} County, ${st}: ${fmt(num(counts.venues))} ${pluralise(num(counts.venues), 'Venue')}`,
    meta: `${fmt(num(counts.venues))} pickleball ${pluralise(num(counts.venues), 'venue')} in ${entry.county} County, ${st}: court counts, lights and addresses, each with its source and the date it was checked.`,
    h1: `Pickleball Courts in ${entry.county} County, ${st}`,
    /* No landmark hero, and no borrowed city photograph. */
    photo: null,
    cardPhoto: courtCardPhoto(`CO/${st}/${bare}`, `${entry.county} County`),
    ...
    editorial: doc
      ? COUNTY_SLOTS.filter(s => doc.slots?.[s.key]).map(s => ({
        ...
        text: doc.slots[s.key],
        paragraphs: washedParagraphs(`CO/${st}/${bare}/${s.key}`, doc.slots[s.key]),
        sources: (doc.sources ?? [])
          .filter(src => (src.supports ?? []).includes(s.key))
          .map(src => ({url: src.url, publisher: src.publisher, retrieved: src.retrieved})),
      }))
...
export function navView() {
  ...
  const groups = [...byState.values()]
    .map(g => ({
      ...
      */
      ...
      /*
        BROWSE BY STATE: the menu shows one link per state and nothing
        under it. Where the link goes depends on what that state has:

          - a published state page (three or more cities, O13): the state page
          - one published city: straight to that city, since a state page
            listing one city would be that city page under another name
          - two cities and no state page: the search page, which answers a
            state name with the cities in it and works with scripts off

        The count beside the name tells the reader which of these to
        expect before they click.
      */
      browseHref: g.href
        ?? (g.cities.length === 1 ? g.cities[0].href : `/search/?q=${encodeURIComponent(g.label)}`),
      cityLabel: `${fmt(g.cities.length)} ${pluralise(g.cities.length, 'city', 'cities')}`,
    }))
    /*
  ...
  return {
    groups,
    /*
      THE GUIDE PAGES ARE ONE MENU, NOT THREE ITEMS.

      They were three, and with the four site links that made seven items
      in the bar. At about 1200px the row no longer fitted beside the brand
      and wrapped onto a line of its own, which is what made the header
      look broken. Adding the scoring, kitchen and net-height pages would
      have made it ten.

      They are also a family: everything here is about the game rather than
      about a place, which is exactly what a submenu is for. Grouping them
      takes the bar from seven items to five and leaves room to add the
      rest without touching this again.
    */
    guides: [
      {href: '/how-to-play-pickleball/', label: 'How to play', blurb: 'The game in ten minutes'},
      {href: '/pickleball-rules/', label: 'Rules', blurb: 'Serving, faults, the kitchen'},
      {href: '/pickleball-scoring/', label: 'Scoring', blurb: 'Who scores, and the three numbers'},
      {href: '/pickleball-court-dimensions/', label: 'Court dimensions', blurb: '20 by 44, and the net'},
    ],
    links: [
      {href: '/about/', label: 'About'},
      {href: '/how-we-verify/', label: 'How we verify'},
      {href: '/add-your-court/', label: 'Add your court'},
    ],
  }
}
...
/*
  THE SEARCH PAGE WHEN THE INDEX CANNOT BE READ.

  /search/ is the only route rendered on demand, so it is the only one that
  can fail at request time rather than at build. When it threw, Next served
  its generic 500 — and a 500 is the worst answer a directory can give:
  decisions.md O9 records a competitor bleeding traffic through exactly
  this, because a crawler reads 500 as "broken, come back later" and KEEPS
  the URL, where a 404 retires it. A reader gets a black page with no way
  onward.

  This is what renders instead: the real heading, the real search box, an
  honest note, and no results — because with no index there are none to
  claim. It is a server-rendered fallback, not an error boundary, so it
  survives the JavaScript-off render Rule 1 requires.

  It hides nothing. The catch logs the error, so the failure is still in
  the function log where it can be diagnosed, and the local checks
  (npm run check:search) still exercise the real path and would fail if the
  real path broke.
*/
export function searchFallbackView(q = '') {
  const query = String(q ?? '').trim()
  return {
    query,
    kind: 'unavailable',
    heading: query ? `Search: ${query}` : 'Search',
    inputValue: query,
    note: 'Search is temporarily unavailable. Every page it would have found is still reachable from the home page, by state and then by city.',
    hasResults: false,
    results: [],
    ...
    suggestions: [],
    /*
      The page prints its own opening sentence before this one, so this
      picks up from there rather than repeating it.
    */
    scaleSentence:
      'That standard is unaffected by this: it is the search index that is unavailable, ' +
      'not the pages themselves, and every one of them is still reachable by state and city.',
  }
}
...
export function searchView(q, filter = null) {
  ...
  return {
    ...
    */
    scaleSentence:
      /*
        Read from the prebuilt search index, not from data.siteTotals().
        Rule 2 is intact — the number still comes from getCounts(), at
        build time, exactly as a prerendered page's numbers do. Calling the
        data layer here would drag data.csv into the one function that is
        rendered on demand, which is what made /search/ fall over in
        production.
      */
      `We have verified ${fmt(venuesTotal(REPO_ROOT))} venues so far. ` +
