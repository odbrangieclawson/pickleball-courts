...
import {cityPhoto, countyPhoto, statePhoto, courtPhoto} from './photos.mjs'
...
/*
  ...
  court count as "not verified yet" it would be incoherent to let a
  ...
  return {
    ...
    */
    ...
    cities: cities.map(c => ({
      ...
      /* A photograph OF that city, so it needs no placeholder marker. Null
         where Commons has nothing usable, and the card renders without one
         rather than borrowing a picture of somewhere else. */
      photo: cityPhoto(c.state, c.slug, c.city),
...
export function stateView(slug) {
  ...
  return {
    stateName: s.stateName,
    state: s.state,
    photo: statePhoto(s.state, s.stateName),
    title: `Pickleball Courts in ${s.stateName}`,
    meta: `${fmt(num(s.counts.venues))} verified pickleball venues across ${fmt(s.cities.length)} ${pluralise(s.cities.length, 'city', 'cities')} in ${s.stateName}, each with its source and the date it was checked.`,
...
export function cityView(stateSlug, citySlug) {
  ...
  return {
    ...
    meta: `${fmt(num(c.counts.venues))} verified pickleball ${pluralise(num(c.counts.venues), 'venue')} in ${c.city}, ${c.state}: court counts, lights and addresses, each with its source and the date it was checked.`,
...
const FILTER_EXCLUSIONS = {
  ...
  outdoor: 'Nothing with only indoor courts appears here. Venues whose outdoor court count we have not verified are also absent, because we do not assume a court is outdoor because we have not been told otherwise.',
...
export function filterView(stateSlug, citySlug, filter) {
  ...
  */
  ...
  return {
    ...
    meta: `${fmt(num(count))} verified pickleball ${pluralise(num(count), 'venue')} in ${c.city}, ${c.state} ${FILTER_PREDICATE_TEXT[filter]}, each with its source and the date it was checked.`,
...
export function countyView(stateSlug, countySlugWithSuffix) {
  ...
  return {
    ...
    meta: `${fmt(num(counts.venues))} verified pickleball ${pluralise(num(counts.venues), 'venue')} in ${entry.county} County, ${st}: court counts, lights and addresses, each with its source and the date it was checked.`,
    h1: `Pickleball Courts in ${entry.county} County, ${st}`,
    photo: countyPhoto(st, entry.county),
