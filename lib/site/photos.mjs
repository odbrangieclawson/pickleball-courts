...
/*
  A COURT PHOTOGRAPH FOR A LONG PROSE SECTION, AT LOW OPACITY.

  The state, city and county notes are the longest reading on this site:
  four sections of unbroken text, capped at the measure, with half the page
  empty beside them. Correct typographically and lifeless to look at.

  This returns a photograph to set into that empty column as a wash. It is
  DECORATION AND NOTHING ELSE, which is why it returns no alt text and no
  `depicts`: the markup renders it aria-hidden with an empty alt, the same
  treatment the home page hero gets and for the same reason. A picture that
  is 16% visible behind nothing is not making a claim about a place, so
  there is nothing for a reader to be misled about and nothing for a screen
  reader to announce.

  It draws from the same fourteen court photographs the venue cards use, so
  it adds no new file, no new licence and no new row to /image-credits/ —
  every one of them is already credited there.

  The key is hashed with a prefix of its own so a section does not get the
  same photograph as a card that happens to share its key, and so the four
  notes on one page differ from the four on the next.
*/
export function proseWash(key) {
  const set = DB.courts ?? []
  if (!set.length) return null
  const p = set[hash(`wash:${String(key)}`) % set.length]
