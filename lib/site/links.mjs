/*
  ...
  A STATE page has no threshold in the brief. O13 asked for one and set it
  at 3 while it was open. The owner closed it at 1 on 2026-09-09: the
  directory grows a city or a venue at a time, and a rule that withholds a
  state's page until its third city withholds it for months at a stretch.

  The worry O13 recorded — that a one-city state page would repeat its city
  page, and that Rule 9 forbids two URLs competing for one intent — is
  answered by the gate that was always the real one. A state page publishes
  only with a written state note whose every slot is filled, at 1,500-2,500
  words (Rule 5, IMMUTABLE). A page carrying that cannot be a copy of a city
  page, because no city page contains it. The count was never what stopped
  a thin page; the note is.

  So this number now says only "at least one city to write about", and
  `linkGraph()` in views.mjs still deletes any state whose note is missing,
  which is what keeps a link from being minted to a page that will not
  build.
*/
...
export const STATE_MIN_CITIES = 1
export const NEAREST_CITIES = 5

/*
  A CAP ON WHAT MAY BE CALLED "NEARBY", WHICH THERE WAS NOT ONE OF.

  nearestCities() took the five closest published cities and stopped. With
  a national directory holding a handful of cities, "closest" is not the
  same as "near": Vancouver, WA's page listed Charlotte, North Carolina —
  3,669 km away — under the heading "Nearby cities we publish", followed by
  Cary and Apex at roughly 3,800 km. Three of the five entries were on the
  other side of the continent, presented to a reader as somewhere to go
  instead.

  250 km is about 155 miles: far enough to keep the real regional pairs
  this directory already has — Vancouver to Portland at 14 km, Seattle to
  Portland at 233, Charlotte to Raleigh at about 200 — and short enough
  that everything it admits is a drive somebody might actually make for a
  game. Past that it is not a nearby city, it is a different part of the
  country.

  The empty case was already written for: the city page says "No other city
  near {city} has three venues yet, so there is nothing to send
  you across to. We would rather leave the space empty than link you to a
  page with nothing on it." That sentence was true of a directory with one
  city and became a lie the moment a second one existed anywhere in the
  United States. It is true again.
*/
export const NEAREST_CITIES_MAX_KM = 250

export const VENUE_ALTERNATIVES = 3

const lower = s => String(s ?? '').toLowerCase()
/* Same rule as citySlug in data.mjs: "St. George" -> "st-george", not "st.-george". */
export const citySlugOf = c => lower(c).replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
...
/**
 * The nearest published cities to this one, by centroid distance, and only
 * those actually within NEAREST_CITIES_MAX_KM. Cities with no coordinates
 * are skipped rather than guessed at, and a city with nothing inside the
 * radius returns an empty list rather than the least-distant continent.
 */
export function nearestCities(graph, state, citySlug, limit = NEAREST_CITIES, maxKm = NEAREST_CITIES_MAX_KM) {
