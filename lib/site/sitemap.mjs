...
import {venuePagePublishes, statePagePublishes} from './views.mjs'

/* One definition for the whole site; see lib/site/origin.mjs. Re-exported
   because app/robots.ts imports it from here. */
...
export function sitemapEntries() {
  ...
  add('/how-to-play-pickleball/', null, 'editorial', 'monthly', 0.6)
  add('/pickleball-court-dimensions/', null, 'editorial', 'monthly', 0.6)
  add('/pickleball-rules/', null, 'editorial', 'monthly', 0.6)
  add('/pickleball-scoring/', null, 'editorial', 'monthly', 0.6)
  add('/about/', null, 'editorial', 'yearly', 0.3)
  add('/image-credits/', null, 'editorial', 'yearly', 0.2)
  add('/how-we-verify/', null, 'editorial', 'yearly', 0.3)
  add('/add-your-court/', null, 'editorial', 'yearly', 0.3)
  add('/privacy/', null, 'editorial', 'yearly', 0.2)
  add('/terms/', null, 'editorial', 'yearly', 0.2)

  for (const [st] of graph.publishedStates) {
    /* An unwritten state page fails Gate 4 and has no page; advertising it
       here would promise a crawler a page we refused to publish. */
