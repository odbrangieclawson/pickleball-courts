...
import {isCount, isPreviewCounts} from '../data/counts.mjs'
import {checkBand} from './words.mjs'
import {checkEditorial, countSpecificSentences, SLOTS_FOR, EDITORIAL_SLOTS} from './editorial.mjs'
...
export function checkPageGates({pageType, counts, html = null, editorial = null, schema = [], venues = [], seenEditorial = new Map()}) {
  ...
  } else if (isPreviewCounts(counts)) {
    /*
      Preview counts are drawn from unverified rows. Letting gate 1 pass on
      them would put a green tick beside "63 verified venues" when none are
      verified - the same vacuously-true signal that made an empty gate 6
      look like a pass.
    */
    g.gate1 = {
      pass: false,
      detail: `counts are UNVERIFIED PREVIEW counts (${counts.venues.value} unverified rows). Rule 8 counts only verified venues, so this is 0 of ${MIN_VERIFIED_VENUES}.`,
    }
  } else if (pageType === 'venue') {
    /*
      A VENUE page has a different threshold, and applying the 3-venue rule
      to it is simply wrong — a venue is one venue by definition, so the
      city rule would refuse every venue page ever built. Gate 1 as written
      in decisions.md §7: "3+ verified venues (city/county/filter); verified
      address and court count (venue)."
    */
    const v = venues[0] ?? null
    const problems = []
    if (!v) problems.push('no venue record')
    else {
      const prov = v.field_provenance ?? {}
      if (!v.street_address) problems.push('no address')
      else if (!prov.street_address) problems.push('address is not verified against a source')
      if (v.total_courts === null || v.total_courts === undefined) problems.push('no court count')
      else if (!prov.total_courts) problems.push('court count is not verified against a source')
    }
    g.gate1 = {
      pass: problems.length === 0,
      detail: problems.length === 0
        ? 'verified address and verified court count'
        : `venue threshold not met: ${problems.join('; ')}`,
    }
  } else {
  ...
  /* ---- Gate 4: word band + three specific sentences ---- */
  {
    ...
    const slots = SLOTS_FOR[pageType] ?? EDITORIAL_SLOTS
    const ed = checkEditorial(editorial, seenEditorial, slots)
    if (!ed.pass) problems.push(...ed.problems)
    const specific = countSpecificSentences(editorial, slots)
