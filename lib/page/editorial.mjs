...
/*
  Venue slots. These are the hardest to fill honestly and the most damaging
  to fake, because the people searching a venue by name already know the
  venue. A wrong court count on a city page is an error; a wrong one on a
  venue page is read by someone standing on the court.
*/
export const VENUE_SLOTS = Object.freeze([
  {key: 'description', prompt: 'Two or three paragraphs about this specific place. What is it actually like, what is it part of, what would someone notice on arrival?'},
  {key: 'getting_there', prompt: 'Parking, transit, and access. Where do you leave the car, which bus, is the entrance obvious?'},
  {key: 'what_to_expect', prompt: 'Peak hours, skill level, open play, whether you will get on, whether you should bring a net.'},
])

/*
  A filter page argues why that filter matters in that city, which is a
  different question from what any single venue is like.
*/
export const FILTER_SLOTS = Object.freeze([
  {key: 'why_this_filter', prompt: 'Why does this filter matter in this city specifically? Year-round play, budget play, evening play — what makes it worth its own page here?'},
  {key: 'what_qualifies', prompt: 'What exactly puts a venue on this list, and what deliberately does not?'},
])
