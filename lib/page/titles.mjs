/*
  ...
  DECISION D8 - VERIFIED COUNTS ONLY IN TITLES (decisions.md §8). Every
  {N} below is a Count from getCounts(), whose denominator is verified
  venues. A city holding 22 imported rows and 6 verified venues gets a
  title that says 6. The imported row count has no route to this file.
...
/**
 * Title: Pickleball Courts in {City}, {ST} - {venues} Venues
 * overflow: drop the count clause entirely
 *
 * The ladder used to have a " Verified" rung between the two. The word left
 * every city, venue, county, filter and state page on 2026-09-09 (see
 * decisions.md change log), so the rung is gone rather than merely unused.
 */
...
/*
  Dropping tokens right-to-left one at a time would leave "- 12" stranded
  without its noun, so the count clause is dropped as a unit. Implemented
  explicitly rather than relying on the generic dropper.
*/
export function cityTitleWithLadder({city, state, counts}) {
  const n = num(counts.venues)
  const base = `Pickleball Courts in ${city}, ${state}`
  const full = `${base} - ${n} ${pluralise(n, 'Venue')}`
...
/**
 * Meta: {venues} pickleball venues in {City}, {ST}: {courts}
...
export function cityMeta({city, state, counts, lastChecked}) {
  ...
  const meta = `${v} pickleball ${pluralise(v, 'venue')} in ${city}, ${state}: ` +
...
export function filterMeta({filter, city, count, distinguishing}) {
  const n = num(count)
  return `${n} of ${count.denominator} ${city} ${pluralise(count.denominator, 'venue')} ` +
...
export function venueTitle({name, city, state}) {
  ...
  /*
    This used to be declared as ONE non-droppable token, which meant the
    overflow model above — the whole reason this module assembles a title
    from a token list rather than a template string — never ran for a
    venue. Any venue whose name pushed the title past 65 characters failed
    the build outright, with an error telling the reader to "shorten a
    fixed token instead" when no fixed token was theirs to shorten.
    Portland's East Portland Community Center comes to 66, over by a single
    character, and Portland Parks & Recreation uses no shorter name for it.

    So the location clause degrades right-to-left, in the order a reader
    loses least by. The state goes first: a venue page already carries it
    in the breadcrumb, the H1, the canonical URL and the meta description,
    so the title is the one place it is purely redundant. The city goes
    only if that is still not enough.

    The name itself is never dropped and never truncated. A name cut
    mid-word is worse than a long title — that is the rule this module was
    written to hold, and it still holds.

    THE GENERIC SUFFIX GOES LAST, AND IT DOES GO.

    Austin's "Austin Tennis and Pickleball Center at Walnut Creek Sports
    Park" is 63 characters on its own. With " - Pickleball Courts" fixed, the
    title was 83 with both location tokens already dropped, and the build
    failed on the largest venue in that city — again telling the reader to
    shorten a fixed token that was not theirs to shorten.

    So the suffix is droppable too, and because assembleTitle() takes the
    RIGHTMOST droppable first, it is the last to go: state, then city, then
    the suffix, then nothing left and a throw. That ordering is the point.
    A title that is just the venue's name is a weaker title and an honest
    one; a build that refuses to publish a real venue because its operator
    gave it a long name is neither.
  */
  return assembleTitle([
    {text: name},
    {text: ' - Pickleball Courts', droppable: true},
    {text: ` in ${city}`, droppable: true},
    {text: `, ${state}`, droppable: true},
  ], TITLE_MAX)
}
